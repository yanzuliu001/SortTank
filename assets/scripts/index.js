        ...
        if (window.tt) {
            if (window.params.getUserParams("openid")) {
                $login.default.login({
                    ...
                    (window.openid = window.params.getUserParams("openid")),
                    window.Logger.info(
                        "## 检查上次是否有异常退出导致在线时长未上报",
                        $timeManage.default.getOnlineTime()
                    ),
                    $timeManage.default.getOnlineTime() > 0 && $report.default.online(),
                    window.Logger.info("## 检查上次是否有异常退出导致事件队列未上报", window.reportQueue.len()),
                    window.reportQueue.len() > 0 && $report.default.reportQueue(),
                            ...
                            $api.getOpenid(
                                ...
                                function (t) {
                                    if (0 == t.data.code) {
                                        ...
                                        $timeManage.default.getOnlineTime() > 0 &&
