...
var s = (function () {
    ...
    t.prototype.checkReportQueue = function (t) {
        ...
        window.Logger.info(
            "## 当前事件队列情况;",
            "事件队列总数（" + window.reportQueue.len() + "）（限制数为：" + $config.LIMIT_REPORT_COUNT + "）",
            "距离首个事件时长（" +
                ($function.getClientTime() - $timeManage.default.getFirstReportClientTime()) +
                "）（限制时长为：" +
                $config.LIMIT_REPORT_INTERVAL +
                "）",
            $timeManage.default.getFirstReportClientTime(),
            window.reportQueue.len() >= $config.LIMIT_REPORT_COUNT ||
                ($timeManage.default.getFirstReportClientTime() &&
                    $function.getClientTime() - $timeManage.default.getFirstReportClientTime() >=
                        $config.LIMIT_REPORT_INTERVAL)
        );
        if (
            window.reportQueue.len() > 0 &&
            (window.reportQueue.len() >= $config.LIMIT_REPORT_COUNT ||
                ($timeManage.default.getFirstReportClientTime() &&
                    $function.getClientTime() - $timeManage.default.getFirstReportClientTime() >=
                        $config.LIMIT_REPORT_INTERVAL))
